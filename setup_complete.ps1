...
Write-Host "`n[3/6] Waiting for containers to be healthy (60 seconds)..." -ForegroundColor Yellow
...
if ($successCount -eq $sqlFiles.Count) {
    ...
    Write-Host "  1. Start data producer:  cd data_producer; pip install -r requirements.txt; python producer.py" -ForegroundColor White
    Write-Host "  2. Start API server:     cd api; pip install -r requirements.txt; uvicorn main:app --reload" -ForegroundColor White
    Write-Host "  3. Start dashboard:      cd dashboard; pip install -r requirements.txt; streamlit run streamlit_app.py" -ForegroundColor White
    Write-Host "`nOr run all at once:       .\run_all.ps1" -ForegroundColor Green
